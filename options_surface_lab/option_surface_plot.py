...
# `with_caption`, `figure_caption` and `as_panel_figure` moved to `page_shell` in T-69.
# They are the figure-side half of the panel contract, not 1.1 content: Assignment 2's
# figures need them and may not reach into this module (AD-12), and `page_shell` was already
# importing `figure_caption` back out of here through a deferred local import to dodge the
# cycle that created. Re-exported so every existing call site — the app, the builder, the
# tests — keeps working, and so that "where does the caption live" has one answer.
from options_surface_lab.page_shell import (  # noqa: E402,F401
    as_panel_figure,
    figure_caption,
    with_caption,
)
