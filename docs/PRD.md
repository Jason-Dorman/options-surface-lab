...
**✅ 2026-09-18 (T-60). The PO wrote all five answers, and the waiver expired by itself.**
The methodology block — the Rule / Fill / Stock / Audit / Limitation hierarchy — is on the
page; the three paragraphs that sat beside it **folded into the answers** the same day, two of
them having restated an answer's own opening sentence and the midpoint example being strictly
weaker than the R² and the $49.50 bid-versus-mid bound that the fill answer now cites.
`OBSERVATION_POINT_FACTS` outlived its paragraph on purpose, so `select_strike` is still
re-run on the two prices the argument was measured from.

*The waiver worked exactly as designed.* FR-19's refusal blocked the deploy on 09-17, was
downgraded to a `::warning::` on 09-18 so the PO could look at the artifact, and
`test_the_write_up_guard_is_as_strict_as_the_write_up_is_finished` went red the moment
`writeup.ANSWERS` was complete — restoring `::error::` + `exit 1` was a failing test, not
someone remembering. **A waiver that cannot expire is a deleted guard with extra steps**, and
this one expired without being trusted to.
