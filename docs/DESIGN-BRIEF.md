...
5. **No visual literal outside `theme.py`**, across `option_surface_plot.py`,
   `options_surface_app.py`, `page_shell.py` and **every** page builder (`build_preview.py`,
   `build_covered_call.py`). The list is `THEMED_SOURCES` in `tests/test_theme.py`, and a
   module that emits markup and is not on it is a module free to hold a colour.
...
| 2026-09-18 | **A page names itself, and the nav is a map (PO).** The wordmark was the *site's* on every page, so a reader landing on `/covered-call/` was told what the site is called and had to read the instrument line to learn what they were looking at; it is `page_shell.PAGE_WORDMARKS` now — **Covered Call Blotter** there, the site's name everywhere else — in the command bar and in the browser tab. And `.osl-nav` lists **every** page with the current one in `ACCENT`, where it had listed only the *others*: a two-page site then showed one link with nothing to say what it was one of, and the cross-links had been there since T-79 without the PO being able to find them. Links at rest moved `TEXT_MUTED` → `TEXT` for the same reason. |
