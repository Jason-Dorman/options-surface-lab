...
| T-41 | **[PO]** Resolve the deployment model with the instructor: (a) frontend on Pages + backend hosted elsewhere via `REFLEX_API_URL`, (b) `reflex deploy` to Reflex Cloud, (c) static Plotly page. Decides FR-9, AD-4 and AD-5, and whether T-14/T-15 matter for deployment at all. | FR-9, AD-4, AD-5 | ☐ |
...
| T-14 | Import-time baking: compute default figures/metrics at module import from the committed pickle; set as State defaults | AD-4 | ☐ |  **Note 2026-08-31:** does not unblock Pages on its own — the export blanks before any baked default is read (AD-4 warning, T-41).
